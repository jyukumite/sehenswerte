...
namespace SehensWerte.Filters
{
    public class FilterChoice : AutoEditor.ValuesAttributeInterface
    {
        ...
        static FilterChoice()
        {
            ...
            m_Factory.Add("WeightedAudioA_100tap_16kHz", (string s) => new FirFilter(FftFilter.GenerateArbitraryFir(
                        width: 100,
                        list: FftFilter.WeightingA,
                        samplesPerSecond: 16000)));
            m_Factory.Add("WeightedAudioC_100tap_16kHz", (string s) => new FirFilter(FftFilter.GenerateArbitraryFir(
                        width: 100,
                        list: FftFilter.WeightingC,
                        samplesPerSecond: 16000)));

            FilterCoefficients.List.Keys
                .Where(key => m_Factory.ContainsKey(key) == false)
                .ForEach(key =>
                    m_Factory.Add(key, (string filter) => new FirFilter(FilterCoefficients.List[filter])));
