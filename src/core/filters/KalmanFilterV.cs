...
{
    // first differential (not acceleration) Kalman filter (see KalmanFilterA for more complex version)
    public class KalmanFilter
    {
        ...
        public double[] Insert(double[] newState, double timeStep = 1.0)
        {
            ...
            m_Estimate = m_Estimate.Add(gain.Product(newState.AsColumn().Subtract(m_H.Product(m_Estimate))));
            m_ErrorCovariance = m_Eye.Subtract(gain.Product(m_H)).Product(m_ErrorCovariance);

            for (int loop = 0; loop < newState.Length; loop++)
            {
                PredictedState[loop] = m_Estimate[loop * 2, 0];
            }

            double[,] phi = Phi(timeStep, length);
        ...
        private double[,] Phi(double timeStep, int length)
        {
            var phi = m_Eye.Copy();
            for (int loop = 0; loop < length; loop++)
            {
