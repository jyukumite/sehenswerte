...
namespace SehensWerte.Controls.Sehens
{
    ...
    {
        private static Skin? SaveImageSkin = null;
        ...
        private class ExportDataForm
        {
            ...
            public ExportType FileType = ExportType.CsvVertical;
        ...
        private static void Export(ScopeContextMenu.DropDownArgs a, ExportType type, ExportDataForm.Destination dest, string filename = "", ExportDataForm? edit = null)
        {
            ...
            if (edit == null)
            {
                ...
                edit = (ExportDataForm?)ImportExportEdit(type, autoEditorForm);
                if (edit != null)
                {
            ...
            if (flag && edit != null)
            {
                waveforms = ExtractWaveformsToSave(a, edit);
            ...
            if (flag && edit != null && waveforms != null)
            {
                switch (type)
                {
                    case ExportType.CsvVertical: SaveCSV(edit, waveforms); break;
                    case ExportType.TsvVertical: SaveCSV(edit, waveforms, "\t"); break;
                    case ExportType.Wav: SaveWAV(edit, waveforms); break;
                    case ExportType.SpaceSeparatedValues: SaveSpaceSeparated(a, edit, waveforms); break;
                    case ExportType.SinglePng: SaveImage(a.Scope, filename); break;
                    case ExportType.MultiplePng: SaveImages(a.Scope, filename); break;
        ...
        private static void SaveImages(SehensControl scope, string filename)
        {
            Skin skin = new Skin();
            skin.CopyMembersFrom(SaveImageSkin ?? scope.ScreenshotSkin);
            using AutoEditorForm autoEditorForm = new AutoEditorForm();
            if (autoEditorForm.ShowDialog("Save", "Info", skin))
            {
                SaveImageSkin = skin;
                string contents = scope.ScreenshotToRtf(skin);
                File.WriteAllText(filename, contents);
            }
        }

        private static void SaveImage(SehensControl scope, string filename)
        {
            Skin skin = new Skin();
            skin.CopyMembersFrom(SaveImageSkin ?? scope.ScreenshotSkin);
            using AutoEditorForm autoEditorForm = new AutoEditorForm();
            ...
            {
                SaveImageSkin = skin;
                scope.ScreenshotToBitmap(skin).Save(filename, ImageFormat.Png);
            }
        }
        ...
        public static void SaveWav(SehensControl scope, string filename, string[] viewNames)
        {
            ...
            if (a.Views.Count > 0)
            {
                ExportDataForm exportData = new ExportDataForm() { FileType = ExportType.Wav, Filename = filename };
